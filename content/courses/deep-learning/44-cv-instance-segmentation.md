...
说起来，前面几章我们已经把卷积网络（第09篇讲过的CNN）能做的事过了一遍。检测那边告诉我们物体在哪里（第39篇讲了Faster R-CNN、YOLO和DETR），语义分割那边告诉我们每个像素属于哪一类。这两件事其实都已经挺难了，可真实世界里有些活儿，光知道这些还不够。
...
要做实例分割，最经典的一条路就是Mask R-CNN。它是在Faster R-CNN（第39篇讲过的两阶段检测器）的基础上，再加一个小分支出来的。
...
当然通用也有代价。SAM在特定任务上的精度，有时候比不过专门训练过的小模型，毕竟它没有专门学过那些领域细节。所以现在大家常用的折中办法，是拿SAM当预训练底座，再在自己的数据上微调一下（fine-tune，用自家数据继续训练使模型更贴合特定任务），既保留了通用性，又照顾了精度。这种思路我们在第16篇讲训练稳定性的时候提过类似的味道，工具是要看场景的，没有一套配方走天下。
