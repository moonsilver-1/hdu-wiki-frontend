---
title: "46：图像分割FCN、U-Net与DeepLab"
date: "2026-07-07"
author: "moonsilver"
...
说起来，前面我们在第30篇讲卷积神经网络的时候，做的是图像分类，整张图最后归到一类就好。后来第42篇讲目标检测，更进一步去框出每个物体的位置。可现实里有些活儿比这还细，光画一个框远远不够。
...
还记得第40篇我们提过分割的几种形式吗。语义分割只区分类别，不区分同类的不同个体。比如画面里有两只猫挨着，语义分割都标成猫这一类，两只猫在它眼里就是一回事。要是想把不同的个体也分开，那叫实例分割（第42篇提过的Mask R-CNN就属于这一路），再往细里走还有把两者合起来的，叫全景分割。这一篇我们重点讲语义分割。

## 2.FCN：把分类网络改造成像素级预测

要做的活儿清楚了，紧接着的问题是怎么做。最直觉的做法是，把第30篇里那种分类网络搬过来用，每个像素周围抠一小块，单独过一遍网络判一次类别。听起来简单，可实际跑起来慢得让人难受，而且对每个像素单独处理，也丢掉了整体结构的信息。
