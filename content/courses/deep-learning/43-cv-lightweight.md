---
title: "43：轻量化网络 MobileNet、ShuffleNet与EfficientNet"
date: "2026-07-04"
author: "moonsilver"
...
说起来，前面第30篇我们聊过卷积神经网络，第41篇又顺着捋了一遍经典分类网络，第42篇讲了目标检测的R-CNN家族。这些网络越做越深、越做越宽，精度是一路往上走的。可你大概也注意到了，它们都有个共同的脾气，就是吃算力、吃内存、吃电量。ResNet也好，Faster R-CNN也好，离了显卡跑起来都费劲。问题是真到了现实里，好多设备根本塞不进一张显卡。
...
我们先回顾一下残差结构。第41篇讲ResNet的时候提过，残差块的做法是先用 $1\times 1$ 卷积把通道压低，再做 $3\times 3$ 卷积，再用 $1\times 1$ 把通道升回去，整体是个两头宽、中间窄的沙漏形状。倒残差刚好反过来，先用 $1\times 1$ 卷积把通道升上去（升得还挺高，比方说升到原来的六倍），再做 $3\times 3$ 的逐通道卷积做空间混合，最后再用 $1\times 1$ 卷积把通道降回来，整体是个两头窄、中间胖的纺锤形状。
...
所以选轻量化模型，不妨记住三步。第一步，在FLOPs相近的若干候选里挑出几个，初筛一遍。第二步，务必把这几个候选拿到目标真机上实测延迟和能耗，这一步省不了。第三步，结合自己的硬约束（电池、散热、延迟上限）做最终选择。光看论文里报的FLOPs和ImageNet精度就拍板，十有八九要踩坑。第42篇讲检测的时候我们提过工程化和论文指标之间的差距，到了轻量化这边，这个差距只会更大。
